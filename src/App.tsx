...
const Legend = () => (
  <div className="legend">
    <h4>Magnitude</h4>
    <div>
      <i style={{ background: '#d9534f' }}></i> 5.0+
    </div>
    <div>
      <i style={{ background: '#f0ad4e' }}></i> 3.0 - 4.9
    </div>
    <div>
      <i style={{ background: '#5cb85c' }}></i> &lt; 3.0
    </div>
  </div>
);
    ...
    <div className="side-panel">
      ...
      <div className="details-grid">
        ...
        <div className="detail-item">
          ...
          <span>{mag !== null ? mag.toFixed(1) : 'N/A'}</span>
        </div>
        <div className="detail-item">
          <strong>🕒 Time</strong>
...
// A simple component for the theme toggle button
    ...
    if (debouncedSearchQuery.length > 2) {
      const fetchSuggestions = async () => {
        try {
          ...
          console.error('Failed to fetch suggestions:', searchError);
        }
      };
      fetchSuggestions();
    } else {
  ...
  const handleSuggestionClick = (suggestion: Suggestion) => {
    ...
    if (suggestion.type === 'country' && suggestion.boundingbox) {
      const [minLat, maxLat, minLon, maxLon] =
        suggestion.boundingbox.map(parseFloat);
      map.fitBounds(
        [
          [minLat, minLon],
          [maxLat, maxLon],
        ],
        { animate: true, duration: 1.5, easeLinearity: 0.2 }
      );
    } else {
      map.flyTo([parseFloat(suggestion.lat), parseFloat(suggestion.lon)], 12, {
        animate: true,
        duration: 1.5,
        easeLinearity: 0.2,
      });
    ...
    <>
      ...
      >
        <LayersControl position="topright">
          ...
          {/* RESTORED: The Topographic Map layer is back */}
        ...
        {/* RESTORED: Both controls are now direct children of the map and will not conflict */}
        ...
        {earthquakes.map((earthquake) => {
          ...
          return (
            <Marker
              key={earthquake.id}
              position={position}
              icon={getMarkerIcon(earthquake.properties.mag)}
              eventHandlers={{
                click: () => {
            ...
            >
              <Popup>
                <b>{earthquake.properties.place}</b>
                <br />
                Magnitude:{' '}
