...
// Custom control component for theme and zoom
const CustomControls = ({ theme, setTheme }: { theme: 'light' | 'dark', setTheme: (theme: 'light' | 'dark') => void }) => {
  const map = useMap();
  return (
    <div className="custom-controls">
      {/* Theme toggle button, now separate */}
      <button 
        onClick={() => setTheme(theme === 'light' ? 'dark' : 'light')} 
        title="Toggle UI Theme"
        className="theme-toggle-button" // Add a class for specific styling
      >
        {theme === 'light' ? '🌙' : '☀️'}
      </button>

      {/* Zoom control buttons inside the leaflet-bar */}
      <div className="leaflet-bar leaflet-control-zoom"> {/* Added leaflet-control-zoom for default Leaflet styling */}
 ...
 // Collapsible Search Control Component
 const SearchControl = (props: {
   searchQuery: string;
   setSearchQuery: (q: string) => void;
   handleSuggestionClick: (s: Suggestion) => void;
   suggestions: Suggestion[];
   handleClearSearch: () => void; // ADD THIS LINE (from instructions)
 }) => {
   ...
   const handleSearch = (e: React.FormEvent) => {
     ...
     if (props.suggestions.length > 0) {
       props.handleSuggestionClick(props.suggestions[0]);
     }
   };

   if (!isOpen) {
     return (
       <button className="search-control-button" onClick={() => setIsOpen(true)} title="Search Location">
         🔍
       </button>
     );
   }
     ...
     <div className="search-panel">
       ...
       <div className="search-wrapper">
         <form onSubmit={handleSearch} className="search-container">
           ...
           {/* NEW: Clear button appears when there is text */}
           {props.searchQuery && (
             <button type="button" className="clear-search-btn" onClick={props.handleClearSearch}>&times;</button>
           )}
           <button type="submit">Go</button>
         </form>
         {props.suggestions.length > 0 && (
           <ul className="suggestions-list">
             {props.suggestions.map((s) => (<li key={s.osm_id} className="suggestion-item" onClick={() => props.handleSuggestionClick(s)}>{s.display_name}</li>))}
           </ul>
         )}
       </div>
     </div>
 ...
 function App() {
   ...
   useEffect(() => {
     console.log("Theme changed to:", theme); // ADD THIS LINE
   ...
   // ADD THIS NEW FUNCTION (from instructions)
   const handleClearSearch = () => {
     setSearchQuery('');
     setSuggestions([]);
     if (map) {
       map.flyTo([20, 0], 3, { // Fly back to the original view
         animate: true,
         duration: 1.5
       });
     }
   ...
   // NEW: Function to handle closing the side panel and resetting the view
   ...
   return (
     <>
       <SidePanel earthquake={selectedEarthquake} onClose={handleCloseSidePanel} />
       
       <MapContainer 
         center={[20, 0]} zoom={3} scrollWheelZoom={true} 
         minZoom={2} style={{ height: "100vh", width: "100vw" }} ref={setMap}
         zoomControl={false}
         noWrap={true}
...
          </LayersControl.BaseLayer>
         ...
         <SearchControl 
           ...
           handleClearSearch={handleClearSearch} // ADD THIS LINE (from instructions)
         />
         <CustomControls theme={theme} setTheme={setTheme} />
         <Legend />
